...
import { 
    ...
    checkSubscription,
    waitForElement 
} from './common.js'; // Path from /js/main.js to /js/common.js

document.addEventListener('DOMContentLoaded', () => {
    let user = null; // This will hold user and subscription data (now includes subscriptionList)

    // --- Initial DOM State Check (for debugging) ---
    console.group('Initial DOM State Check');
    ...
    // --- App Initialization Logic (Main Page) ---
    console.log('--- Initializing App Logic (Main Page) ---');
    // On the main page, try to get token from localStorage (set by callback.js)
    // or handle old hashes if any exist.
    handleTokenFromHash();

    checkSubscription()
        .then(userData => { // Capture the returned user data
            user = userData; // Assign the fetched user data to the local 'user' variable
            updateUI(); // Now update the UI, knowing the definitive user state
        })
        .catch(err => {
            console.error('App initialization error (subscription check failed):', err);
            updateUI(); // Fallback to update UI even if check fails (user will be null)
        });
});